...
export interface AdminProduct { id: number; name: string; price: number; categoryIds: number[]; taxIds: number[]; available: boolean; storable: boolean; favorite: boolean; description: string; hasImage: boolean; dinerAttributes: DinerAttributes }
...
interface RawTemplate { id: number; name: string; list_price: number; pos_categ_ids: number[]; taxes_id: number[]; available_in_pos: boolean; is_storable: boolean; is_favorite: boolean; description_sale: string | false; image_128: string | false; diner_attributes: string | false }
interface RawCategory { id: number; name: string; sequence: number; kitchen_station: string | false }

const TEMPLATE_FIELDS = ['name', 'list_price', 'pos_categ_ids', 'taxes_id', 'available_in_pos', 'is_storable', 'is_favorite', 'description_sale', 'image_128', 'diner_attributes']
