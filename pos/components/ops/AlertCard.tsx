...
interface AlertCardProps { alert: Alert; late: number; queue: number; onResolve: (alert: Alert, text: string) => void }

export function AlertCard({ alert, late, queue, onResolve }: AlertCardProps) {
  const t = useTranslations('pos.ops')
  const busy = alert.severity === 'busy'
  const table = alert.tableNumber ?? '—'
  const body = alert.kind === 'kitchen'
    ? t('alertBody.kitchen', { ref: `#${alert.orderId}`, table, late, queue })
