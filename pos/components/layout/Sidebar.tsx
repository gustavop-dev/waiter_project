    ...
    <aside className="w-sidebar shrink-0 bg-sidebar text-sidebar-soft p-5 flex flex-col gap-5">
      <div className="flex items-center gap-2.5 px-2">
        ...
        <div className="flex flex-col leading-tight">
          <span className="font-bold text-xl tracking-[-0.035em] text-sidebar-ink">{t('brand')}<span className="text-brand-500">.</span></span>
      ...
      <nav className="flex flex-col gap-0.5">
        {ITEMS.map((item) => {
          ...
          if (href) {
            return (
              <Link key={item} href={href} className={cn(itemClass, item === active ? 'bg-brand-500 text-ink font-bold' : 'hover:bg-sidebar-hover hover:text-sidebar-ink')}>
                {t(`nav.${item}`)}
              </Link>
            )
          }
