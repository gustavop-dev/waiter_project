...
export function Shell({ mode, active = 'operation', badges, subnav, autonomy, children }: ShellProps) {
  ...
  return (
    <div className="h-screen flex bg-canvas">
      {mode === 'sidebar'
        ? <Sidebar active={active} restaurant={restaurant} shift={shift} userName={userName} badges={badges} subnav={subnav} autonomy={autonomy} />
        : <Rail active="tables" userName={userName} />}
