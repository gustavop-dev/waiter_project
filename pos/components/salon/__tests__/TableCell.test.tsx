...
it('renders the state word next to the table number', () => {
  wrap(<TableCell view={{ table, state: 'billing', total: 48800, tax: 7791, orderId: 9, startedAt: null, waiter: null }} selected={false} onSelect={jest.fn()} now={NOW} />)
...
it('omits the amount when the table is free', () => {
  wrap(<TableCell view={{ table, state: 'free', total: 0, tax: 0, orderId: null, startedAt: null, waiter: null }} selected={false} onSelect={jest.fn()} now={NOW} />)
...
it('shows elapsed time and a progress bar for an occupied table', () => {
  wrap(<TableCell view={{ table, state: 'occupied', total: 74200, tax: 11851, orderId: 9, startedAt: '2026-09-04 20:00:00', waiter: 'Alejandra' }} selected={false} onSelect={jest.fn()} now={NOW} />)
