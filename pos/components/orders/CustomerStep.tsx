...
export function CustomerStep({ info, onChange, onContinue }: { info: CustomerInfo; onChange: (patch: Partial<CustomerInfo>) => void; onContinue: () => void }) {
  ...
  return (
    <div className="p-8 flex justify-center">
      <div className="w-[672px] max-w-full flex flex-col gap-5">
        ...
        <fieldset className="flex flex-col gap-2">
          ...
          <div role="radiogroup" aria-label={t('orderType')} className="flex gap-3 flex-wrap">
            {ORDER_TYPES.map((type) => (
              <button key={type} type="button" role="radio" aria-checked={info.type === type} onClick={() => onChange({ type })}
                className={cn('w-[200px] h-[52px] px-4 rounded-md border bg-surface flex items-center gap-2.5 text-[16px] font-semibold text-ink',
                  info.type === type ? 'border-primary' : 'border-border')}>
                <Icon name={TYPE_ICON[type]} size={20} className="text-soft" />
                <span className="flex-1 text-left">{t(`types.${type}`)}</span>
                <span className={cn('w-5 h-5 rounded-full border-2 grid place-items-center', info.type === type ? 'border-primary' : 'border-border')}>
                  {info.type === type && <span className="w-2.5 h-2.5 rounded-full bg-primary" />}
                </span>
              </button>
            ))}
          </div>
        ...
        <Button variant="primary" className="self-start mt-2 rounded-lg" disabled={!customerInfoValid(info)} onClick={onContinue}>
