...
export function DataTable<T>({ columns, rows, rowKey, emptyText, onRowClick, selectedKey }: {
  ...
  return (
    <div className="flex flex-col min-h-0">
      <div role="row" className="grid gap-3 px-[22px] py-3.5 bg-canvas border-b border-[#EFE9E0] text-[13px] font-medium tracking-[0.08em] uppercase text-ink-3" style={{ gridTemplateColumns: template }}>
        ...
        {rows.map((row, i) => {
          ...
          return (
            <Tag key={key} role="row" type={onRowClick ? 'button' : undefined} onClick={onRowClick ? () => onRowClick(row) : undefined}
              className={cn('w-full grid gap-3 px-[22px] py-4 items-center border-b border-[#F3EFE8] text-base text-left', i % 2 === 1 && 'bg-[#FDFCFA]',
