...
import { LOGO_TYPES, imageDataUrl, resizeImage, validateLogoFile, type LogoFileError } from '@/lib/domain/image'
...
export function BrandForm({ restaurantName }: { restaurantName: string }) {
  ...
  async function onFile(e: ChangeEvent<HTMLInputElement>) {
    ...
    try { setChange({ base64: await resizeImage(file) }); setLogoError(null) } catch { setLogoError('decode') }
  }
  ...
  const text = (key: TextKey, label: string, extra: { placeholder?: string; hint?: string } = {}) => (
    <TextInput key={key} label={label} value={b[key]} maxLength={LIMITS[key]} placeholder={extra.placeholder} onChange={(e) => patch({ [key]: e.target.value })}
      hint={<span className="flex justify-between gap-3"><span>{extra.hint}</span><span className="font-mono tabular shrink-0">{b[key].length}/{LIMITS[key]}</span></span>} />
  )
        ...
        <div className="flex min-w-0 max-w-md flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            <div className="flex items-end gap-3">
              ...
              <TextInput label={t('hex')} value={b.color} onChange={(e) => setColor(e.target.value)} placeholder={DEFAULT_COLOR} maxLength={7} spellCheck={false} autoCapitalize="characters" className="w-40 font-mono uppercase" />
            </div>
            {readout}
          </div>
          ...
          <fieldset className="flex flex-col gap-2">
            ...
            <div className="flex items-center gap-4">
              <div className="grid h-20 w-40 shrink-0 place-items-center overflow-hidden rounded-[10px] border border-border bg-surface px-2 text-center text-[11px] uppercase tracking-[0.08em] text-ink-3">
                {shownLogo
              ...
              <div className="flex flex-col gap-2">
                <input ref={fileRef} type="file" accept={LOGO_TYPES.join(',')} aria-label={t('logoFile')} onChange={onFile} className="text-sm" />
            ...
            {logoError && <p role="alert" className="text-[13px] text-busy-ink">{t(`logoError.${logoError}`)}</p>}
          </fieldset>
          <SaveBar state={state} onSave={onSave} disabled={!hexOk || !contrastOk} />
        </div>
