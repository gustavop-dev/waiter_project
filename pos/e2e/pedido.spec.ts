...
test('a waiter opens a table, adds two burgers, sends to kitchen and charges', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /^Mesa 3: Disponible/ }).click()
  ...
  await expect(page.getByRole('button', { name: /^Mesa 3: En progreso/ })).toBeVisible()
  await page.getByRole('button', { name: /^Mesa 3: En progreso/ }).click()
  await page.getByRole('button', { name: /^Cobrar \$ 87\.822$/ }).click()
  await page.getByRole('button', { name: 'Agregar pago' }).click()
  await page.getByRole('button', { name: 'Confirmar cobro' }).click()
  await page.getByRole('button', { name: 'Cerrar' }).click()
  await expect(page.getByRole('button', { name: /^Mesa 3: Disponible/ })).toBeVisible()
})
