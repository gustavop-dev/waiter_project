...
test('an order sent from one device can be charged from another', async ({ browser }) => {
  ...
  await a.getByRole('button', { name: /^Mesa 7: Disponible/ }).click()
  ...
  await b.getByRole('button', { name: /^Mesa 7: En progreso/ }).click()
  ...
  await expect(b.getByRole('button', { name: /^Mesa 7: Disponible/ })).toBeVisible()
  await cashier.close()
})
