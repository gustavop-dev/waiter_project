...
// Requiere la siembra del kit: node scripts/seed-pantry.cjs (recetas de 6 platos, 8 ingredientes, 2 proveedores).
...
test('the menu shows the servings of a dish and its recipe in the detail modal', async ({ page }) => {
  ...
  await expect(dialog.getByRole('listitem').filter({ hasText: 'Carne de res molida' })).toContainText('150 g')
...
test('adds an ingredient with category, unit and supplier', async ({ page }) => {
  ...
  await dialog.getByRole('radio', { name: 'Distribuidora La Cosecha' }).click()
  ...
  await expect(row).toContainText('Distribuidora La Cosecha')
})
...
test('requests an ingredient and sees it in the request list', async ({ page }) => {
  ...
  await expect(row).toContainText('Carnes y Mar del Valle')
