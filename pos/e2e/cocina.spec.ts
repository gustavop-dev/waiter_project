...
test('kitchen marks a ticket ready and served; the salon shows it served and charges it', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /^5\b.*Libre/ }).click()
  ...
  await expect(page.getByRole('button', { name: /^5\b.*En cocina/ })).toBeVisible()
  ...
  await page.getByRole('button', { name: /^5\b.*Servido/ }).click()
  await page.getByRole('button', { name: /^Cobrar \$ 87\.822$/ }).click()
  await page.getByRole('button', { name: 'Agregar pago' }).click()
  await page.getByRole('button', { name: 'Confirmar cobro' }).click()
  await page.getByRole('button', { name: 'Cerrar' }).click()
  await expect(page.getByRole('button', { name: /^5\b.*Libre/ })).toBeVisible()
})
