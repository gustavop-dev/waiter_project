...
test('the waiter searches a dish, adds a general kitchen note and the KDS shows it', async ({ page }) => {
  ...
  await page.getByRole('button', { name: /^6\b.*Libre/ }).click()
  ...
  await expect(page.getByRole('button', { name: /^1\b.*Libre/ })).toHaveCount(0)
  await page.getByRole('button', { name: /^6\b.*Servido/ }).click()
