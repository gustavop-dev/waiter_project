...
# ---------------------------------------------------------------------------
# Registro central (ADR 2026-09-05-registro-minimo-tokens-y-credenciales)
# ---------------------------------------------------------------------------
REGISTRY_INTERNAL_KEY = os.getenv("REGISTRY_INTERNAL_KEY", "")
# Credenciales de inquilinos cifradas en reposo. En producción la clave es
# obligatoria y distinta del SECRET_KEY; en desarrollo se deriva de él.
REGISTRY_FERNET_KEY = os.getenv("REGISTRY_FERNET_KEY", "")
if not REGISTRY_FERNET_KEY:
    if IS_PRODUCTION:
        raise RuntimeError("REGISTRY_FERNET_KEY es obligatoria en producción")
    import base64
    import hashlib

    REGISTRY_FERNET_KEY = base64.urlsafe_b64encode(hashlib.sha256(SECRET_KEY.encode()).digest()).decode()
