...
// Falla si B1 / familia B / los patrones base se quedan sin layout (las familias los registran en sus bloques; mientras no llegan,
// el genérico ocupa su sitio) o si las pantallas fijas dejan de exportarse.
it('registers B1 / family B / base patterns and exports the fixed screens', () => {
  expect(MENU_LAYOUTS.B1).toBeDefined()
  expect(CART_LAYOUTS.B).toBeDefined()
  expect(PAY_LAYOUTS.B).toBeDefined()
  expect(SIGNUP_PATTERNS.banner5).toBeDefined()
  expect(CODE_PATTERNS.casillas).toBeDefined()
  expect(HISTORY_PATTERNS.porMes).toBeDefined()
