...
import { AddButton, FTabs, MenuEmpty, ReferenceNote, SearchField, SearchToggle } from '@/components/templates/families/F/parts'
...
// cabecera y al pie, y las celdas 2×2 son los platos de la categoría (atributos.compartimentos no existe en el contrato 2,
// así que el rótulo en versalitas de cada celda es su precio en mono y el contenido su nombre «· N piezas» si hay dato).
// Con más de cuatro platos, la cuarta celda es «Elige» (borde discontinuo) y despliega el resto como filas. Tocar una celda la
// selecciona (borde negro); tocarla de nuevo, o el título de la cabecera, abre el plato (onOpen). El CTA negro añade el
// seleccionado (onAdd). Sin barra de pedido ni ＋ por plato en el marco: la barra fija de Waiter cubre el pedido.
export function F2Menu({ entry, query, setQuery, category, setCategory, onOpen, onAdd }: MenuLayoutProps) {
          ...
          : (
            <div className="px-5 py-[18px] grid grid-cols-2 gap-3">
              {cells.map((d) => {
                ...
                return (
                  <button key={d.id} type="button" aria-pressed={active} onClick={() => tap(d)} className={`min-h-[118px] rounded-t-tarjeta bg-t-superficie p-3.5 flex flex-col justify-between text-left ${active ? 'border-2 border-t-tinta' : 'border border-t-borde'} ${d.agotado ? 'opacity-55' : ''}`}>
                    <span className="font-t-mono tabular text-[12px] tracking-[0.1em] uppercase text-t-tinta-terciaria font-medium">{d.agotado ? t('common.soldOut') : formatCop(d.precio)}</span>
                    <span className="text-[16px] font-medium leading-[1.25]">{label(d)}</span>
                  </button>
                )
              })}
              {rest.length > 0 && (
                <button type="button" aria-expanded={expanded} aria-controls="f2-rest" onClick={() => setExpanded((e) => !e)} className="min-h-[118px] rounded-t-tarjeta bg-t-superficie border border-dashed border-t-tinta-terciaria/60 p-3.5 flex flex-col justify-between text-left">
                  <span className="text-[12px] tracking-[0.1em] uppercase text-t-tinta-terciaria font-medium">{tf('bento.choose')}</span>
            ...
            {rest.map((d) => {
              ...
              return (
                <li key={d.id} className="border-b border-t-borde last:border-b-0 flex items-center gap-3">
                  <button type="button" aria-pressed={active} onClick={() => tap(d)} className={`flex-1 min-w-0 min-h-12 py-2.5 flex items-center justify-between gap-3 text-left ${d.agotado ? 'opacity-55' : ''}`}>
                    <span className={`text-[15px] truncate ${active ? 'font-medium' : ''}`}>{label(d)}</span>
