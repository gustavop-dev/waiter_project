...
// picanteAjustable, que no existe: se omite. Añadidos de Waiter: pestañas de categoría y lupa arriba, ＋ de 44 px por fila.
// Sin barra de pedido en el marco: la barra fija de Waiter cubre el pedido.
...
export function F4Menu({ entry, query, setQuery, category, setCategory, onOpen, onAdd }: MenuLayoutProps) {
  ...
  const chip = (on: boolean) => `h-[38px] px-3.5 rounded-t-chip text-[14px] ${on ? 'bg-t-acento text-t-acento-tinta font-medium' : 'border border-t-borde text-t-tinta'}`
...
function Row({ dish, reason, onOpen, onAdd }: { dish: Dish; reason: Filter | null; onOpen: (d: Dish) => void; onAdd: (d: Dish) => void }) {
  ...
  const spicy = a?.picante
  const dim = reason !== null || dish.agotado
  return (
    <li className={`px-5 py-3.5 border-b border-t-borde last:border-b-0 flex items-start gap-3 ${reason ? 'opacity-45' : dish.agotado ? 'opacity-55' : ''}`} data-filtered={reason ? 'true' : undefined}>
      <button type="button" onClick={() => onOpen(dish)} className="flex-1 min-w-0 flex flex-col text-left">
        ...
        {dish.agotado && <span className="text-[13px] font-medium text-busy-ink mt-1.5">{t('soldOut')}</span>}
      </button>
      {!dim && <AddButton dish={dish} onAdd={onAdd} circle="w-[30px] h-[30px] rounded-full bg-t-acento text-t-acento-tinta text-[16px]" />}
    </li>
  )
}
