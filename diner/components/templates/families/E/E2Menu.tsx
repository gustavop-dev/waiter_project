...
// E2 · Coctelería en fichas (docs/diseno/plantillas/E2): cabecera con la marca en la voz de la plantilla (Instrument Serif 25) y
// subtítulo gris (el lema de la marca o «N cócteles»); bajo la cabecera, el buscador y las categorías de Waiter; fichas de una columna
// sobre superficie violeta (radio 16, padding 15) con nombre 18/700 y precio mono 16 en la misma línea, la descripción como
// ingredientes en 14 y chips de perfil de 28 px (atributos.etiquetas, solo si existen). Barra inferior «N ítems · total» con el botón
// en acento: el marco dice «Otra igual» (repite la última ronda), pero la sesión no guarda rondas, así que lleva a orderBarHref como
// «Ver pedido». El marco no dibuja ＋: va uno de 32 px con borde al final de la ficha (44 px de toque); tocar la ficha abre el plato.
// La página aún superpone su OrderBar flotante cuando hay ítems (integración pendiente).
export function E2Menu({ entry, query, setQuery, category, setCategory, onOpen, onAdd, cart, orderBarHref }: MenuLayoutProps) {
    ...
    <div className="flex flex-col min-h-[60vh] text-t-tinta">
      ...
      <section role="tabpanel" aria-labelledby={tabId(category)} className="flex-1 px-5 py-4">
        {dishes.length === 0
          ...
          : <ul className="flex flex-col gap-3">{dishes.map((d) => <Card key={d.id} dish={d} onOpen={onOpen} onAdd={onAdd} />)}</ul>}
      </section>
...
// Ficha: nombre + precio, ingredientes (descripción), chips de perfil (etiquetas) y ＋. Agotado: ficha al 55 % con la insignia.
function Card({ dish, onOpen, onAdd }: { dish: Dish; onOpen: (d: Dish) => void; onAdd: (d: Dish) => void }) {
  ...
  return (
    <li className={`rounded-t-tarjeta bg-t-superficie p-[15px] flex flex-col gap-2 ${dish.agotado ? 'opacity-55' : ''}`}>
      <button type="button" onClick={() => onOpen(dish)} className="text-left flex flex-col gap-2">
      ...
      <div className="flex items-center gap-1.5 flex-wrap min-h-[28px] -mb-1.5">
        {dish.agotado && <span data-testid="sold-out-badge" className="inline-flex items-center h-[28px] px-2.5 rounded-t-chip bg-t-borde text-[12px] font-medium text-busy">{t('soldOut')}</span>}
        {tags.map((x) => <span key={x} className="inline-flex items-center h-[28px] px-2.5 rounded-t-chip bg-t-borde text-[12px]">{x}</span>)}
        <AddButton dish={dish} onAdd={onAdd} className="ml-auto -my-3 -mr-2" />
      </div>
    </li>
  )
}
