...
const B1_SPEC = join(__dirname, '..', '..', '..', '..', 'docs', 'diseno', 'plantillas', 'B1', 'spec.json')
...
it('embeds B1 exactly as its spec.json describes it', () => {
...
it('maps every token to a --t-* css variable', () => {
  ...
  expect(Object.keys(vars).sort()).toEqual([
    '--t-acento', '--t-acento-suave', '--t-acento-tinta', '--t-borde', '--t-cuerpo', '--t-display', '--t-display-peso', '--t-display-tracking', '--t-display-transform',
    '--t-fondo', '--t-mono', '--t-radio-boton', '--t-radio-chip', '--t-radio-tarjeta', '--t-superficie', '--t-tinta', '--t-tinta-suave', '--t-tinta-terciaria',
  ])
  expect(vars['--t-acento']).toBe('#C1873A')
  expect(vars['--t-display']).toBe("'Ubuntu', system-ui, sans-serif")
  expect(vars['--t-display-peso']).toBe('700')
  expect(vars['--t-radio-chip']).toBe('999px')
  expect(vars['--t-mono']).toBe("'IBM Plex Mono', ui-monospace, monospace")
})
...
it('loads each google font once and skips the ones the layout already ships', () => {
  ...
  expect(fontsToLoad(t)).toEqual(['Bebas Neue', 'Space Grotesk'])
  expect(fontsToLoad(DEFAULT_TEMPLATE)).toEqual([])
  document.head.innerHTML = ''
  expect(applyGoogleFonts(t)).toEqual(['Bebas Neue', 'Space Grotesk'])
  expect(loadGoogleFonts(['Bebas Neue'])).toEqual([])
  const links = Array.from(document.head.querySelectorAll('link[rel="stylesheet"]'))
  expect(links.map((l) => l.id)).toEqual(['gf-bebas-neue', 'gf-space-grotesk'])
